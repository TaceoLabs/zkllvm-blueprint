...
void test_fixedpoint_gather_acc_inner(FixedType acc,
    ...
    using component_type =
        blueprint::components::fix_gather_acc<ArithmetizationType,
                                              BlueprintFieldType,
    ...
    auto result_check = [&expected_res, &expected_res_f, acc, data, index_a, index_b](
    ...
    component_type component_instance(
        witness_list, std::array<std::uint32_t, 1>({0}), std::array<std::uint32_t, 0>(), index_b);
...
void test_fixedpoint_gather_acc(RngType &rng) {
...
void field_operations_test() {
    ...
    for (std::size_t i = 0; i < RandomTestsAmount; i++) {
        test_fixedpoint_gather_acc<FixedType>(seed_seq);
    }
}
